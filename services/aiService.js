...
请直接给出分析结果，不要加入AI风格的客套话，也不要使用表情符号。分析要细致到句式结构层面，让人能够完全理解并复制这种写作模式。`;
  ...
  buildGeneratePrompt(originalContent, newTopic, model, showThinking, useDeepAnalysis) {
    if (model === 'claude') {
      if (showThinking) {
        return `我有一篇关于某个产品的小红书爆文，现在需要你分析它的结构，然后用相同的句式和结构写一篇关于新主题的爆文。请按照下面的格式输出：
...
${useDeepAnalysis ? '请投入更多思考，确保新文案在保持原文结构的同时，针对新主题进行了最合适的调整，质量极高。' : ''}`;
        ...
        return `我有一篇关于某个产品的小红书爆文，现在需要你分析它的结构，然后用相同的句式和结构写一篇关于新主题的爆文。
...
1. 分析原始爆文的结构、句式和写作技巧
2. 使用完全相同的结构和句式，创作一篇关于新主题"${newTopic}"的小红书爆文
...
最终输出只需要提供完整的新爆文，不要包含分析过程，不要使用表情符号，保持语言风格与原文一致。`;
      ...
      return `分析下面这篇小红书爆文的结构和表达方式，然后按照完全相同的结构写一篇关于新主题的文章。
...
直接输出新文章内容，不要包含分析部分。不使用表情符号，文风与原文保持一致。`;
  ...
  async processRequest(content, actionType, model, options = {}) {
    ...
    if (actionType === 'analyze') {
      ...
      prompt = this.buildGeneratePrompt(content, newTopic, model, showThinking, useDeepAnalysis);
    }
